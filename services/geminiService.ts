...
export const generateReply = async (
  ...
  // Inject any prompts registered via setExtensionPrompt() before extensions run
  try { flushExtensionPromptsInto(mutableMessages); } catch (e) { console.warn('flushExtensionPromptsInto failed', e); }

  // Notify generation_started so listeners can react before the prompt is locked in
  try { await globalEventBus.emit(EVENT_TYPES.GENERATION_STARTED, { messages: mutableMessages }); } catch (e) { console.warn(e); }
