...
const ChatPage: React.FC<Props> = ({ settings }) => {
  ...
  useEffect(() => {
    if (view === 'chat' && character) {
        ...
        // SillyTavern parity: emit CHAT_CHANGED with the new chat id so ST
        // extensions that key off chat id (Megumin, Memory, etc) refresh.
        globalEventBus.emit(EVENT_TYPES.CHAT_CHANGED, character.id).catch(console.error);
        globalEventBus.emit(EVENT_TYPES.CHAT_LOADED, character.id).catch(console.error);
    }
  ...
  const processResponse = async (fullHistory: Message[], userInput: string, hiddenDirection?: string) => {
      ...
      try {
        ...
        // SillyTavern parity: notify extensions that a character message was rendered
        globalEventBus.emit(EVENT_TYPES.CHARACTER_MESSAGE_RENDERED, botMsg).catch(console.error);
        globalEventBus.emit(EVENT_TYPES.GENERATION_ENDED, botMsg).catch(console.error);
  ...
  const handleSendMessage = async (overrideContent?: string, hiddenDirection?: string) => {
    ...
    if (textToSend.trim()) {
        ...
        // ST parity: USER_MESSAGE_RENDERED fires once the user message reaches the DOM
        globalEventBus.emit(EVENT_TYPES.USER_MESSAGE_RENDERED, userMsg).catch(console.error);
    }
  ...
  const confirmDeleteMessage = async () => {
      if (msgToDelete && character) {
          const deletedMsg = messages.find(m => m.id === msgToDelete);
          const newMsgs = messages.filter(m => m.id !== msgToDelete);
          setMessages(newMsgs);
          await deleteMessage(character.id, msgToDelete);
          // ST parity: extensions key off MESSAGE_DELETED to recompute state
          if (deletedMsg) globalEventBus.emit(EVENT_TYPES.MESSAGE_DELETED, deletedMsg).catch(console.error);
          setMsgToDelete(null);
      }
  };
